...
/*
func listAvailableWifi() [][]string {
	var wifiList [][]string
	cmd := execCommand("nmcli -t --fields SSID,SIGNAL,SECURITY dev wifi")
	for i:=0; i < len(cmd.message); i++ {
...
func makeButtonList() []fyne.CanvasObject {
	...
	//wifiList := listAvailableWifi()
	//for i := 1; i <= len(wifiList); i++ {
	//	//index := i // capture
	//	// TO-DO call for password
	//	wifiButton := widget.NewButton(wifiList[i-1][0] + "", func() {
	//		
	//	})
