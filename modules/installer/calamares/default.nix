...
pkgs.stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = with pkgs; [ cmake extra-cmake-modules qt5.qtbase patchelf];
  buildInputs = with pkgs; with libsForQt5; with xorg;[
    boost kparts.dev kpmcore.out kservice.dev kirigami2
    libatasmart libxcb yaml-cpp libpwquality parted polkit-qt python3
    qtbase qtquickcontrols qtsvg qttools qtwebengine.dev util-linux qt5.wrapQtAppsHook
  ];

  cmakeFlags = [
    "-DPYTHON_LIBRARY=${pkgs.python3}/lib/lib${pkgs.python3.libPrefix}.so"
    "-DPYTHON_INCLUDE_DIR=${pkgs.python3}/include/${pkgs.python3.libPrefix}"
  ...
  postPatch = ''
    ...
    sed -e 's,/usr/share/zoneinfo,${pkgs.tzdata}/share/zoneinfo,' \
        -i src/modules/locale/SetTimezoneJob.cpp \
        -i src/libcalamares/locale/TimeZone.cpp

    sed -e 's,/usr/share/X11/xkb/rules/base.lst,${pkgs.xkeyboard_config}/share/X11/xkb/rules/base.lst,' \
        -i src/modules/keyboard/keyboardwidget/keyboardglobal.cpp

    sed -e 's,"ckbcomp","${pkgs.ckbcomp}/bin/ckbcomp",' \
